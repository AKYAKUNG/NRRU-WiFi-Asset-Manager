...
export default function LoginForm() {
  ...
  // 1. เพิ่ม State สำหรับเปิด-ปิด Modal สำเร็จ
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:81/db-NRRU-WiFi-Asset-Manager/login.php', {
        method: 'POST',
      ...
      if (data.success) {
        ...
        // 2. แสดง Pop-up ล็อกอินสำเร็จ และรอ 1.5 วินาทีก่อนเปลี่ยนหน้า
  ...
  const hasUsername = Boolean(formData.username && formData.username.trim().length > 0);
  const hasPassword = Boolean(formData.password && formData.password.trim().length > 0);
  ...
  return (
    <div className="relative">
      ...
      {/* 3. Pop-up Modal แสดงสถานะเข้าสู่ระบบสำเร็จ */}
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md animate-fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-6 sm:p-8 max-w-sm w-full text-center shadow-2xl transform transition-all scale-100">
            <div className="w-16 h-16 bg-emerald-100 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 rounded-full flex items-center justify-center mx-auto mb-4 border border-emerald-200 dark:border-emerald-800 animate-bounce">
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 font-medium mb-6">
              ยินดีต้อนรับเข้าสู่ระบบ NRRU Wi-Fi Asset Manager <br />
              กำลังนำท่านไปยังหน้าจัดการ...
            </p>

            <div className="flex items-center justify-center gap-2 text-xs font-semibold text-blue-600 dark:text-blue-400">
              <Loader2 className="w-4 h-4 animate-spin" />
              <span>กำลังดาวน์โหลดข้อมูลผู้ใช้</span>
            </div>
          </div>
        </div>
      )}
